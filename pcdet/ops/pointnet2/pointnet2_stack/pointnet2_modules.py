...
class VectorPoolLocalInterpolateModule(nn.Module):
    ...
    def forward(self, support_xyz, support_features, xyz_batch_cnt, new_xyz, new_xyz_grid_centers, new_xyz_batch_cnt, weights):
        """
        Args:
...
class VectorPoolAggregationModule(nn.Module):
    def __init__(
        ...
        self.ReshapeWeight1 = nn.Sequential(
            nn.Conv1d(cur_num_c, 1, kernel_size=1, groups=1, bias=False),
            nn.BatchNorm1d(1),
            nn.ReLU()
        )
        self.ReshapeWeight2 = nn.Sequential(
            nn.Conv1d(1, cur_num_c, kernel_size=1, groups=1, bias=False),
            nn.BatchNorm1d(cur_num_c),
            nn.ReLU()
        )
    ...
    def vector_pool_with_local_interpolate(self, xyz, xyz_batch_cnt, features, new_xyz, new_xyz_batch_cnt, weights):
        """
        Args:
            xyz: (N, 3)
        ...
        voxel_features = self.local_interpolate_module.forward(
            ...
            new_xyz=new_xyz, new_xyz_grid_centers=voxel_centers, new_xyz_batch_cnt=new_xyz_batch_cnt, weights=weights
        )  # ((M1 + M2 ...) * total_voxels, C)

        voxel_features = voxel_features.contiguous().view(-1, self.total_voxels * voxel_features.shape[-1])
        return voxel_features

    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features, weights, **kwargs):
        """
        :param xyz: (N1 + N2 ..., 3) tensor of the xyz coordinates of the features
        :param xyz_batch_cnt: (batch_size), [N1, N2, ...]
        :param new_xyz: (M1 + M2 ..., 3)
        ...
        # if weights != None:
        #     # print(features.shape, torch.cat((weights), 1).squeeze(0).shape)
        #     weights_torch = torch.cat((weights), 1).squeeze(0).unsqueeze(1).expand(features.shape[0], features.shape[1]).cuda()
        ...
        elif self.local_aggregation_type == 'local_interpolation': # 插值
            vector_features = self.vector_pool_with_local_interpolate(
                xyz=xyz, xyz_batch_cnt=xyz_batch_cnt, features=features,
                new_xyz=new_xyz, new_xyz_batch_cnt=new_xyz_batch_cnt, weights=weights # 分配权重
            )  # (M1 + M2 + ..., total_voxels * C)
        else:
            raise NotImplementedError
        ...
        # if weights != None:
        #     new_features = self.ReshapeWeight1(new_features)
        #     # print(new_features.shape)
        #     weights_torch = torch.cat((weights), 1).unsqueeze(1).expand(1, new_features.shape[1], new_features.shape[2]).cuda()
        #     new_features = new_features * weights_torch
        #     new_features = self.ReshapeWeight2(new_features)
        #     # new_features_weighted_fg = new_features_weighted[:, :,  torch.cat((weights), 1).squeeze(0)< 1]
        #     # new_features_weighted_attentioned_fg = self.SelfAttention(new_features_weighted_fg)
        #     # new_features_weighted[:, :, torch.cat((weights), 1).squeeze(0)< 1] = new_features_weighted_attentioned_fg
        #     # new_features_cat = torch.cat((new_features, new_features_weighted), 1)
        #     # new_features_cat = self.PointNet0(new_features_cat)
        #     # part1 = self.PostMlp_PART1(new_features)
        #     # new_features = self.PoatMlp_PART2(part1 + new_features_cat)
        
#         if weights != None and len(weights) != 0:
#             # weights = torch.cat((weights), 1).squeeze(0)
#             # # print(weights.shape, new_features.shape)
#             # new_features_selected = self.ReshapeWeight1(new_features) # size: [1, channel, keypoints_numbers]
#             # # print(new_features.shape)
#             # weights_torch = weights.unsqueeze(0).unsqueeze(0).expand(1, new_features_selected.shape[1], new_features_selected.shape[2]).cuda()
#             # new_features_selected = new_features_selected * weights_torch
#             # new_features = self.ReshapeWeight2(new_features_selected)
            
#             weights_torch = torch.cat((weights), 1).squeeze(0).unsqueeze(0).unsqueeze(0).expand(1, new_features.shape[1],
#                                                                                    new_features.shape[2]).cuda()
#             new_features = new_features * weights_torch
