...
class VectorPoolLocalInterpolateModule(nn.Module):
    ...
    def forward(self, support_xyz, support_features, xyz_batch_cnt, new_xyz, new_xyz_grid_centers, new_xyz_batch_cnt): # need weights
        """
        Args:
        ...
        dist_recip = 1.0 / (dist + 1e-8)
        # print(dist_recip.shape, new_xyz.shape)
        # print(dist_recip[-1], dist_recip[-2])
        norm = torch.sum(dist_recip, dim=-1, keepdim=True)
...
class VectorPoolAggregationModule(nn.Module):
    def __init__(
        ...
        if self.local_aggregation_type == 'local_interpolation': # need weights
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features, **kwargs):
        ...
        elif self.local_aggregation_type == 'local_interpolation': # need weights
